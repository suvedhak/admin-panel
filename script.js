...
for (let i = 0; i < data.length; i++) {
    ...
    generateUser [i] = new User(data[i]);
...
for (let x = 0; x < data.length; x++) {
    ...
    $(`#id-${generateUser[x].idnum}`).click(function() {
...
document.getElementById("search-box").addEventListener('input', function(e) {
    ...
    if(inputVal === "") {
        ...
        for (let a = 0; a < data.length; a++) {
            $(`#id-${data[a].id}`).css("display","block");
        }
        ...
        for (let b = 0; b < filterData.length; b++) {
            $(`#id-${filterData[b].id}`).css("display","block");
            // console.log(filterData[b].id);
        }
    }
    });
